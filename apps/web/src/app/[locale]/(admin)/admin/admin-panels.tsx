...
const chainEnv = parseChainEnv(
  typeof process !== "undefined" ? process.env.NEXT_PUBLIC_CHAIN_ENV : "mainnet",
);

type AdminHandlers = {
  onSaveConfig: (config: AdminConfig & { textModelApiKey?: string }) => Promise<void>;
  ...
  return (
    <section className="space-y-4">
      <p className="text-sm text-muted">
        {t("chainAdapter")}: {overview.chainAdapter}
        {overview.enabledChains?.length
          ? ` · ${overview.enabledChains.join(", ")}`
          : null}
      </p>
      ...
      <div className="flex flex-col gap-2 sm:flex-row">
        ...
        <Select
          ...
          options={[
            ...
            { value: "recruiting", label: t("taskStatus.recruiting") },
            { value: "paused", label: t("taskStatus.paused") },
            { value: "full", label: t("taskStatus.full") },
            { value: "ended", label: t("taskStatus.ended") },
            { value: "taken_down", label: t("taskStatus.taken_down") },
          ]}
        />
      </div>
      {filtered.length === 0 ? (
...
export function AdminConfigPanel({
  ...
  config: AdminConfig;
  onSave: (config: AdminConfig & { textModelApiKey?: string }) => Promise<void>;
}) {
  const t = useTranslations("admin");
  const [local, setLocal] = useState({
    ...config,
    erc20Confirmations: config.erc20Confirmations ?? 12,
  });
  ...
  return (
    <section className="space-y-3 rounded-2xl border border-line bg-surface p-5">
      <p className="text-sm text-muted">
        {t("chainAdapter")}:{" "}
        {local.enabledChains?.length
          ? local.enabledChains.join(", ")
          : local.chainAdapter}
        {local.chainAdapter === "mock" ? ` (${t("chainAdapterMock")})` : null}
      </p>
      <FormField label={t("platformFeeBps")}>
        <Input
          type="number"
      ...
      <FormField label={t("minDepositUsdt")}>
        <Input
          type="number"
          step="0.01"
      ...
      <FormField label={t("minWithdrawUsdt")}>
        <Input
          type="number"
          step="0.01"
      ...
      <FormField label={t("withdrawNetworkFeeUsdt")}>
        <Input
          type="number"
          step="0.01"
      ...
      <FormField label={t("trc20Confirmations")}>
        <Input
          type="number"
      ...
      <FormField label={t("erc20Confirmations")}>
        <Input
          type="number"
          value={local.erc20Confirmations}
          onChange={(event) => setNumber("erc20Confirmations", event.target.value)}
        />
      </FormField>
      <CheckboxField
        label={t("referralEnabled")}
        checked={local.referralEnabled}
        onChange={(event) =>
          setLocal({ ...local, referralEnabled: event.target.checked })
        }
      />
      <FormField label={t("referralEarnBps")}>
        <Input
          type="number"
      ...
      <FormField label={t("referralPublishBps")}>
        <Input
          type="number"
      ...
      <div className="space-y-3 border-t border-line pt-4">
        <p className="text-sm font-medium text-foreground">{t("textModelSection")}</p>
        <p className="text-xs text-muted">{t("textModelHint")}</p>
        <FormField label={t("textModelBaseUrl")}>
          <Input
            type="url"
            value={local.textModelBaseUrl}
            placeholder="https://api.openai.com/v1"
            onChange={(event) =>
              setLocal({ ...local, textModelBaseUrl: event.target.value })
            }
          />
        </FormField>
        <FormField label={t("textModelName")}>
          <Input
            type="text"
            value={local.textModelName}
            placeholder="gpt-4o-mini"
            onChange={(event) =>
              setLocal({ ...local, textModelName: event.target.value })
            }
          />
        </FormField>
        <FormField label={t("textModelApiKey")}>
          <Input
            type="password"
            value={apiKeyInput}
            placeholder={
              local.textModelApiKeyConfigured
                ? local.textModelApiKeyMasked || t("textModelApiKeyConfigured")
                : t("textModelApiKeyPlaceholder")
            }
      ...
      <Button
        ...
        onClick={() =>
          void onSave({
            ...local,
            ...(apiKeyInput.trim()
              ? { textModelApiKey: apiKeyInput.trim() }
              : {}),
          })
        }
      >
