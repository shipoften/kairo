...
export default function NewTaskPage() {
  ...
  const freezeCents = unitPriceCents * totalQuota;
  ...
  return (
    <main className="mx-auto w-full max-w-xl space-y-6">
      <h1 className="font-[family-name:var(--font-display)] text-3xl">{t("create")}</h1>
      <form onSubmit={onSubmit} className="space-y-4 rounded-2xl border border-line bg-surface p-6">
        ...
        <label className="block space-y-1 text-sm">
          ...
          <input type="number" className="w-full rounded-xl border border-line px-3 py-2" value={unitPriceCents} onChange={(e) => setUnitPriceCents(Number(e.target.value))} />
        </label>
        <label className="block space-y-1 text-sm">
          <span>{t("fieldQuota")}</span>
          <input type="number" className="w-full rounded-xl border border-line px-3 py-2" value={totalQuota} onChange={(e) => setTotalQuota(Number(e.target.value))} />
        </label>
        <div className="rounded-xl bg-surface p-4 text-sm">
          <p className="font-medium">{t("freezeSummary")}</p>
          <p className="mt-1 text-muted">
            {t("freezeAmount", { amount: (freezeCents / 100).toFixed(2) })}
          </p>
        ...
        <Button type="submit">{t("createAndPublish")}</Button>
      </form>
    </main>
  );
}
