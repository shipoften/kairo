...
function languageLabelKey(tag: string): "languages.en" | "languages.zh" {
  if (tag === "zh") return "languages.zh";
  return "languages.en";
}
      ...
      {data.items.length === 0 ? (
        ...
        <ul className="space-y-2">
          {data.items.map((task) => {
            ...
            const locales = task.availableLocales?.length
              ? task.availableLocales
              : [];
            return (
              <li
                key={task.id}
