...
import { Chain, getExplorerTxUrl, parseChainEnv } from "@xs-share/shared";
...
import { formatWalletDate, ledgerTypeKey } from "@/lib/wallet/wallet-status";
...
function explorerUrlForTx(txHash: string) {
  const chainEnv = parseChainEnv(process.env.NEXT_PUBLIC_CHAIN_ENV);
  const chain = txHash.startsWith("0x") ? Chain.ERC20 : Chain.TRC20;
  return getExplorerTxUrl(chain, txHash, chainEnv);
}
                ...
                <div className="min-w-0 space-y-1">
                  ...
                  {item.txHash ? (
                    <a
                      ...
                      href={explorerUrlForTx(item.txHash)}
                      target="_blank"
                      rel="noreferrer"
                    >
