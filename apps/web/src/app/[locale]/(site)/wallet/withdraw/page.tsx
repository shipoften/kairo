...
export default function WithdrawPage() {
  ...
  function validate(): string | null {
    if (!isValidChainAddress(parseChain(chain), toAddress)) {
  ...
  const addressLabel =
    chain === Chain.ERC20 ? t("toAddressErc20") : t("toAddress");
  const addressPlaceholder =
    chain === Chain.ERC20
      ? t("toAddressPlaceholderErc20")
      : t("toAddressPlaceholder");
  ...
  return (
    <main className="mx-auto w-full max-w-md space-y-6">
      ...
      <ChainSwitcher value={chain} onChange={setChain} disabled={submitting} />
      ...
      <form onSubmit={onSubmit} className="space-y-4 rounded-2xl border border-line bg-surface p-6">
        ...
        <div className="space-y-1">
          <Label htmlFor="toAddress">{addressLabel}</Label>
          <Input
            id="toAddress"
            type="text"
            className="font-mono"
        ...
        {!loading && withdrawals.length === 0 ? (
          ...
          withdrawals.map((item) => {
            ...
            const statusLabel = t.has(statusKey)
              ? t(statusKey)
              : item.status;
            const itemChain = parseChain(item.chain);
            return (
              <Card key={item.id} className="space-y-2 px-4 py-3 text-sm">
                ...
                <p className="text-muted">
                  {itemChain.toUpperCase()} · {t("netPayout")}:{" "}
                  {displayUsdt(item.netPayoutMicros)}
                </p>
                ...
                {item.txHash ? (
                  <a
                    ...
                    href={getExplorerTxUrl(itemChain, item.txHash, chainEnv)}
                    target="_blank"
                    rel="noreferrer"
                  >
                    {t("viewOnExplorer")}
